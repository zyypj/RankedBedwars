...
public class Game {
	...
	public Game(List<Player> players, Queue queue) {
		...
		try {
			...
			Embed embed = new Embed(EmbedType.ERROR, "Something went wrong...", "O jogo não foi criado pois Something went wrong. Por favor, saia e entre novamente na fila. Se isso continuar acontecendo, contacte o @zyypj", 1);
	...
	public void sendGameMsg() {
		...
		if (remainingPlayers.isEmpty()) {
			...
			embed.setTitle("Game `#" + number + "` começou!");
			...
			if (casual) {
				embed.setDescription("Você entrou numa fila casual, não contará estatísticas");
			}
	...
	public void scoreGame(List<Player> winningTeam, List<Player> losingTeam, Member scoredBy) {
		...
		Embed embed = new Embed(EmbedType.SUCCESS, "Game `#" + number + "` foi pontuado", "", 1);
