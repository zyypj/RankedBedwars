...
public class BanCmd extends Command {
    ...
    public void execute(String[] args, Guild guild, Member sender, TextChannel channel, Message msg) {
        ...
        if (player.ban(unban, reason)) {
            Embed embed = new Embed(EmbedType.DEFAULT, "`" + player.getIgn() + " has banned`", "Use `=fix` when the time limit is up to remove the ban\n" +
                    "If you think it was a wrong ban, open a ticket", 1);
            ...
            Embed success = new Embed(EmbedType.SUCCESS, "", "You have been banned <@!" + ID + "> por `" + duration + "`\n**Reason:** " + reason, 1);
